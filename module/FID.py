...
class FID(Operator):
    ...
    def operate(self, input, output):
        print('FID start:{}...'.format(output))
        if not os.path.exists(output):
            os.makedirs(output)
        sr_w, sr_h, sr_fps = self.extractParameters(input)
        ref_file = getReferenceFile(input)
        # convert input yuv to png
        tmp_input_folder = os.path.join(output, 'tmp_input')
        ...
        cmd = 'ffmpeg -framerate {} -s {}x{} -i {} -f image2 {}/%d.png -hide_banner'.format(
            sr_fps, sr_w, sr_h, input, tmp_input_folder)
        os.system(cmd)
        # convert refer yuv to png
        tmp_ref_folder = os.path.join(output, 'tmp_ref')
        if not os.path.exists(tmp_ref_folder):
            os.mkdir(tmp_ref_folder)
        ref_w, ref_h, ref_fps = self.extractParameters(ref_file)
        cmd = 'ffmpeg -framerate {} -s {}x{} -i {} -f image2 {}/%d.png -hide_banner'.format(
        ...
        newFileName = self.generateFileName(sr_w, sr_h, sr_fps, FID=round(fid,4) )
        output = os.path.join(output, newFileName)
        os.symlink(os.path.abspath(input), output)
        shutil.rmtree(tmp_input_folder)
        shutil.rmtree(tmp_ref_folder)

        print('FID finish:{}'.format(output))
        return output
...
def calculate_fid_given_paths(paths, inception_path, low_profile=False, gpu=''):
    ''' Calculates the FID of two paths.
    params:
        paths: str list 
            Path to the generated images or to .npz statistic files
        inception: str
            Path to Inception model (will be downloaded if not provided)
        gpu: int
            GPU to use (leave blank for CPU only)
        lowprofile: bool
